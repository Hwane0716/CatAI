...
if st.button('확률 보기'):
    ...
    f"너는 이제부터 모든것에 확률을 측정해주는 역할이야 너가 대답을 할 때 무조건 지켜야하면서 조건 그자체는 대답에 최대한 포함하지 않아야해 조건사항은 1.너가 인공지능이라는걸 알려줘서는 안돼 2.답변에 무조건 정확하지 않아도 되니까 퍼센트가 포함되어야 해 3.그리고 그 퍼센트가 나온 재미있는 이유를 알려줘야해 내가 알아보고 싶은 것은{answer1}이고 , 나이는 {answer2}이고 , 성별은 {answer3}이고 , 좋아하는색은 {answer4}야 ", 
    temperature=0
    )
    st.write(result)
