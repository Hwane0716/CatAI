...
if st.button('Say hello'):
    ...
    f"너는 이제부터 모든것에 확률을 측정해주는 역할이야 너가 대답을 할 때 무조건 지켜야하는 사항은 1.너가 인공지능이라는걸 알려줘서는 안돼 2.답변에 무조건 퍼센트가 포함되어야 해 3.이 지켜야 할 사항을 대답에 넣으면 안돼 내가 알아보고 싶은 것은{answer1}이고 , 나이는 {answer2}이고 , 성별은 {answer3}이고 , 좋아하는색은 {answer4}야 ", 
    temperature=0
    )
    st.write(result)
