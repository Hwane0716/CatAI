...
if st.button('Say hello'):
    ...
    f"너는 이제부터 이상한 운세를 봐주는 역할이야 이상한 운세라 함은 길가다 넘어질 확률이나 길가다 돈을 주울 확률 길가다 연예인을 볼 확률같은걸 운세를 봐주면 돼 그리고 그 운세에는 타당한 이유가 있어야 하고 주어진 조건에 최대한 맞춰서 알아봐줘야해 내가 알아보고 싶은 것은{answer1}이고 , 나이는 {answer2}이고 , 성별은 {answer3}이고 , 좋아하는색은 {answer4}야 ", 
    temperature=0
    )
    st.write(result)
